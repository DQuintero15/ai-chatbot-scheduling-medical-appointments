...
const dentistryFirstVisitFlow = addKeyword(EVENTS.ACTION)
  .addAction(
    async (ctx, { state, flowDynamic, gotoFlow, endFlow, extensions }) => {
      ...
      const schedules =
        await scheduleService.getSchedulesAvailablesBySpecialtyID(2);
      ...
      if (!schedules.length) {
        flowDynamic([
          `Ups! Parece que no hay citas disponibles para odontología. ¿Quieres intentar con otra especialidad?`,
        ]);
        return endFlow();
      }
    ...
    async (ctx, { state, flowDynamic, gotoFlow, endFlow, extensions }) => {
      ...
      await flowDynamic(
        [
          ...
          "Recuerda tener una buena higiene bucal y cepillarte los dientes antes de tu cita. ¡Nos vemos pronto! 🦷🤗",
        ],
...
export { dentistryFirstVisitFlow };
